...
function renderDatasets() {
  const rawLabel = dataStore.source.rawDataLoaded
    ? `, 원자료 ${dataStore.source.rawDatasetCount}개 적재`
    : "";
  $("dataBadge").textContent = `${dataStore.source.totalCatalogRows}개 중 ${dataStore.source.selectedRows}개 핵심 데이터${rawLabel}`;
...
function simulateRoute(profile, route, seedOffset = 0) {
  ...
  for (let run = 0; run < runs; run += 1) {
    for (let i = 0; i < agents; i += 1) {
      const localDemand = clamp(region.demand + (rng() - 0.5) * 0.18, 0.18, 0.92);
      const competition = clamp(region.competition + (rng() - 0.5) * 0.16, 0.15, 0.9);
